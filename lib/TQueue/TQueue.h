 ...
 *  \author Petyovský
 *  \version 2022
 *  $Id: TQueue.h 1611 2022-03-01 15:12:26Z petyovsky $
 */
...
typedef uint8_t TQueueElement;					///< Definice typu QueueElement (datový typ elementů fronty)
#define QUEUE_MAXCOUNT 128					///< Maximální počet elementů fronty
...
struct TQueue
	{
	size_t iPushPos;						///< Aktuální index prvku pole určeného pro vložení elementu do fronty
	size_t iPopPos;							///< Aktuální index prvku pole určeného pro odstranění elementu z fronty
 ...
 *  \details Funkce (predikát) vracející \c char hodnotu reprezentující test, zda je fronta prázdná.
 ...
 *  \details Funkce (predikát) vracející \c char hodnotu definující platnost iterátoru.
...
static inline void queue_for_each(struct TQueueIterator aIter, void(*aOperation)(const struct TQueueIterator *aIter))
	{
	for(char valid = queue_iterator_is_valid(&aIter); valid; valid = queue_iterator_to_next(&aIter))
		aOperation(&aIter);
	}
 ...
 *  \param[in] aIter Ukazatel na existující iterátor, jenž je předem asociovaný se zvolenou frontou a který tak definuje počáteční element pro zvolenou operaci
 *  \param[in] aPredicate Ukazatel na predikátovou funkci (funkci vracející \c char a mající jeden parametr typu ukazatel na iterátor)
 ...
 */
static inline struct TQueueIterator queue_find_if(struct TQueueIterator aIter, char(*aPredicate)(const struct TQueueIterator *aIter))
	{
	for(char valid = queue_iterator_is_valid(&aIter); valid; valid = queue_iterator_to_next(&aIter))
		if(aPredicate(&aIter))
			return aIter;
 ...
 */
static inline struct TQueueIterator queue_find_if_not(struct TQueueIterator aIter, char(*aPredicate)(const struct TQueueIterator *aIter))
	{
	for(char valid = queue_iterator_is_valid(&aIter); valid; valid = queue_iterator_to_next(&aIter))
		if(!aPredicate(&aIter))
			return aIter;
